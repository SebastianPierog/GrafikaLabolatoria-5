...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    private void draw(GL2 gl2) {
        ...
        switch(objectNumber){
            case 1:piramidka(gl2,7,11,7);
                break;
            case 2:korkociag(gl2,15,0,255,0, 'X');
                break;
            case 3:korkociag(gl2,15,0,0,255, 'Y');
                break;
            case 4:korkociag(gl2,15,150,75,0, 'Z');
                break;
        }
    }
    // tworzymy 1 scianke potem wywolujemy ją 11 razy
    ...
    void piramidka(GL2 gl2, float r, int N,float h) {
        ...
        for(int i = 0 ; i<N-1;i++)
        {
            ...
            scianka(gl2,xPoints[i], yPoints[i],0,0, xPoints[i+1], yPoints[i+1],h);
            gl2.glPopMatrix();
        }
        gl2.glPushMatrix();
        gl2.glRotatef(360/N,0,1,0);
        scianka(gl2,xPoints[N-1], yPoints[N-1],0,0, xPoints[0], yPoints[0],h);
        gl2.glPopMatrix();
    }
    // 3F bo sa 3 plaszczyzny
    void korkociag(GL2 gl2,int N, int r, int g, int b,char os)
    {
        float[] xPoints = new float[30];
        float[] yPoints = new float[30];
        for(int i = 1 ; i<=30;i++) {
            xPoints[i - 1] = (float) (1 * Math.sin((2 * Math.PI / 30) * i));
            yPoints[i - 1] = (float) (1 * Math.cos((2 * Math.PI / 30) * i));
        }
        gl2.glPushMatrix();
        gl2.glColor3f(r, g, b);
        float wielkosc = 1.0f;
        ...
        for (int i = 0;i<N;i++) {
            for (int j = 0; j < 30; j++){
                gl2.glPointSize(wielkosc);
                gl2.glBegin(GL_POINTS);
                gl2.glPushMatrix();
                gl2.glRotatef(12, 0, 1, 0);
                switch(os) {
                    case 'X': gl2.glVertex3f(ilosc,xPoints[j], yPoints[j]);
                        break;
                    case 'Y': gl2.glVertex3f(xPoints[j], ilosc, yPoints[j]);
                        break;
                    case 'Z': gl2.glVertex3f(xPoints[j], yPoints[j],ilosc);
                        break;
                }
                gl2.glPopMatrix();
